...
SEARCH_KEYWORD=$(jq -r '.searchKeyword' "$CONFIG_FILE")
...
scan_directory() {
  ...
  while IFS= read -r item; do
    ...
    if [ "$item_type" = "file" ]; then
      ...
      if grep -q "$SEARCH_KEYWORD" "$item" 2>/dev/null; then
        content_match="match"
      fi
    fi
    ...
    results+=("$(jq -n \
      ...
      --arg error "$error" \
      --arg match "$content_match" \
      '{name: $name, path: $path, type: $type, extension: $ext, size: ($size | tonumber), timestamp: $time, error: $error, contentMatch: $match}')")
...
echo "Content Analysis complete. Scanned ${#results[@]} items."
save_results